...
def setupArgs() -> None:
  parser = argparse.ArgumentParser(
    description="Download WRF downsampled data and clip to region and save as zarr. See https://dept.atmos.ucla.edu/sites/default/files/alexhall/files/aws_tiers_dirstructure_nov22.pdf"
  )
  parser.add_argument(
    "--model",
    required=True,
  ...
  parser.add_argument(
    "--historical", default=False, type=bool, help="Whether to select historical version of model"
  )
  parser.add_argument(
    "--parameters",
    type=str,
...
def formatWrfArray(
  ...
  wrf_data = wrf_data.assign_coords(lat=lat, lon=lon, hgt=hgt).rename(
